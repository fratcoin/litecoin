...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Fratcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Fratcoin Signed Message:\n";
...
{
    // Fratcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Fratcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 6.9 * 24 * 60 * 60; // Fratcoin: 3.5 days
static const int64 nTargetSpacing = 5.0 * 60; // Fratcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Fratcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Fratcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Fratcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Total Frat Move 18/Mar/2014 ECH Tower? Prankster Steals “T” From Georgia Tech Landmark";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("8dfa16611db9870e600af3f9f7a7ed736c942f5583e785af4e53c52ff363639c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Fratcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FratcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FratcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FratcoinMiner(CWallet *pwallet)
{
    printf("FratcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fratcoin-miner");
    ...
    try { loop {
        ...
        printf("Running FratcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FratcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FratcoinMiner, pwallet));
}
